...
and loss modulus ``G''`` both rise across 10-100+ Hz, a power-law / fractional response).
A source oscillating at the beta content corner (~20 Hz) therefore drives the tissue
through its *dynamic* modulus, not its DC stiffness: the in-phase displacement is
attenuated and phase-lagged relative to the lossless static value.
...
Like ``eta`` and ``kappa`` upstream, the relaxation parameters are reviewer-contestable
inputs measured away from the operating point; the module's job is to make the
frequency-dependence *representable*, not to assert its exact value. The applied factor
is stored on :class:`~base_neural_model.base.types.MechanicsParams` as
...
# Brain viscoelastic relaxation: ms-scale, broad (power-law) spectrum. Contestable, and
# measured away from the beta operating point (like C_v for eta).
DEFAULT_RELAXATION_TIME_S: float = 3.0e-3  # ~3 ms relaxation
DEFAULT_SPRINGPOT_EXPONENT: float = 0.3    # power-law exponent (MRE loss tangent ~0.2-0.4)
...
def viscoelastic_provenance(f_hz: float, params: ViscoelasticParams) -> tuple[str, ...]:
    ...
    return (
        ...
        f"relaxation tau = {params.relaxation_time_s * 1e3:.3g} ms, exponent a = "
        f"{params.exponent} -> |H(f_c={f_hz:.3g} Hz)| = {mag:.3g} (contestable, measured "
        "away from the beta operating point like C_v for eta)",
    )
